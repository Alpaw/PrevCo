...
public class SQLConnector {
		...
		public void createTables() {
			...
			try {
				...
				CreateTables tables = new CreateTables(stmt);
				tables.create();
				tables.insert();
		...
		public ArrayList<NotificationBean> getAllNotifs(int idUser){
			...
			try {
				while(rs.next()) {
					...
					notif.setAutoId(rs.getInt("AutoId"));
		...
		public void ajouterNotif(int userId1, int userId2, int type) {
			System.out.println("type : "+type);
			String req="INSERT INTO notifications (`userId1`, `userId2`, `type`, `createdAt`, `autoId`) VALUES ('"+userId1+"','"+userId2+"','"+type+"',CURRENT_TIMESTAMP, 0)";
			
			doUpdate(req);
		}
		
		public int getIdNotif(NotificationBean nb) {
			
			String rq="SELECT * FROM notifications WHERE autoId="+nb.getAutoId();
			ResultSet rs= doRequest(rq);
			try {
